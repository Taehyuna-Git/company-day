...
export function MyCompanies(){
  ...
  return <section className="my-companies"><h3>My 관심 기업 <span className="muted">{follows.length}</span></h3>
    ...
    <form className="group-create" onSubmit={async e=>{e.preventDefault();await addGroup(name);setName('')}}><input aria-label="새 그룹 이름" placeholder="그룹 이름 · 예: 거래처" maxLength={40} required value={name} onChange={e=>setName(e.target.value)}/><button disabled={busy||!name.trim()}>그룹 만들기</button></form>
