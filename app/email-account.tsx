...
export function AccountContent(){
  ...
  return <div className="account-content">
    ...
    <details className="notice"><summary>개인정보 이용 안내</summary><p>기업의 날은 회원 인증을 위해 이메일을, 개인 설정을 위해 표시 이름·관심 기업·그룹·알림 수신 주소를 저장합니다. 인증과 데이터 저장에는 Supabase를 사용하며 데이터베이스는 일본 도쿄에 있습니다. 인증 메일은 Gmail 발신 계정을 통해 보냅니다. 기념일·뉴스 메일은 별도 신청 시에만 발송합니다. 계정 삭제 시 개인 설정과 관심 기업도 삭제됩니다.</p></details>
  </div>;
}
function Settings({user,accessToken,verificationToken}:{user:AccountUser;accessToken:string;verificationToken:string}){
  const client=getAccountClient()!;
  ...
  async function load(){
    ...
    setName(profile?.display_name||'');setAnniversary(preferences!.anniversary_enabled);setNews(preferences!.news_enabled);setDays(preferences!.lead_days);setHour(preferences!.send_hour);setRecipient(email?.email||'');setPending(verification&&new Date(verification.expires_at).getTime()>Date.now()?verification.email:'');setReady(true);
  }
  ...
  return <div className="account-content"><span className="account-badge">로그인됨</span><p className="account-email">{user.email}</p>
    {verificationToken&&!confirmed&&<div className="account-verification"><p>이 주소를 알림 수신 이메일로 사용할까요?</p><button className="oauth" disabled={busy} onClick={()=>perform(async()=>{await api('confirm',{token:verificationToken});setConfirmed(true);await load()},'알림 이메일 인증을 완료했습니다.')}>이메일 인증 완료</button></div>}
    {!ready?<button disabled={busy} onClick={()=>perform(load,'계정 설정을 불러왔습니다.')}>다시 불러오기</button>:<>
    <form className="account-form" onSubmit={e=>{e.preventDefault();perform(async()=>{const a=await client.from('profiles').update({display_name:name.trim()}).eq('user_id',user.id);if(a.error)throw Error('이름 저장에 실패했습니다.');const b=await client.from('notification_preferences').update({anniversary_enabled:anniversary,news_enabled:news,lead_days:days,send_hour:hour}).eq('user_id',user.id);if(b.error)throw Error('알림 설정 저장에 실패했습니다.')},'설정을 저장했습니다.')}}>
      <label>표시 이름<input maxLength={80} value={name} onChange={e=>setName(e.target.value)} disabled={busy}/></label>
    ...
    <hr/><p>현재 알림 이메일<br/><strong>{recipient||'인증된 수신 주소 없음'}</strong></p>{pending&&<p className="notice">인증 대기: {pending}</p>}
