...
type VerificationDeps={site:string;configured:()=>boolean;verifyUser:(token:string)=>Promise<string|null>;reserve:(user:string,email:string,hash:string,id:string)=>Promise<void>;confirm:(user:string,hash:string)=>Promise<void>;finish:(id:string,status:'sent'|'failed',provider?:string)=>Promise<void>;send:(mail:Mail)=>Promise<string>};
export function createVerificationHandler(d:VerificationDeps){return async(request:Request)=>{
  ...
  if(!['request','confirm'].includes(action||''))return reply({error:'주소를 찾을 수 없습니다.'},404);
  const auth=request.headers.get('Authorization')||'';
  if(!auth.startsWith('Bearer ')||auth.length>16000)return reply({error:'로그인이 필요합니다.'},401);
  ...
  if(action==='confirm'){
    ...
    try{await d.confirm(user,await hashToken(body.token));return reply({ok:true})}
    catch(error){return /invalid_or_expired_token/.test(String(error))?reply({error:'인증 링크가 만료되었거나 이미 사용되었습니다. 변경을 요청한 계정인지 확인하고, 필요하면 새 인증 메일을 받아 주세요.'},400):reply({error:'인증 상태를 확인하지 못했습니다. 내 계정의 현재 알림 이메일을 확인해 주세요.'},503)}
  }
  ...
  const intro='알림을 받을 이메일 변경을 요청하셨습니다. 아래 링크에서 변경을 요청한 계정으로 로그인한 뒤 인증을 완료해 주세요.';
