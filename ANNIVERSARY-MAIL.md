...
현재 상태 (2026-09-20): 코드와 자동 검증 완료. Supabase 신규 스키마·확인된 기념일 13곳·Cron 1개 및 anniversary-mail 함수 배포 완료. 사용자의 명시적 승인에 따라 legacy JWT 검증 옵션을 끄고 함수 내부 인증으로 전환했습니다. SMTP_PASSWORD는 사용자가 저장했으며 공개 상태 응답은 configured=true, enabled=false입니다. 사용자가 승인한 테스트 1회를 시도했지만 발송 확인을 받지 못했고, 사용자도 미수신을 확인했습니다. 이후 메일을 보내지 않는 연결 점검에서 Gmail AUTH PLAIN / EAUTH / SMTP 534 인증 거절을 확인했습니다. 사용자에게 올바른 Gmail 앱 비밀번호로 수정하도록 요청한 상태입니다. 실제 수신 검증 전까지 자동 발송은 비활성화 상태를 유지합니다.
