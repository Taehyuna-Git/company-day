...
export async function accountApi(request:Request,env:AccountEnv):Promise<Response>{
  ...
  if(!['/api/account/email/request','/api/account/email/confirm','/api/account/email/cancel'].includes(url.pathname))return response({error:'주소를 찾을 수 없습니다.'},404);
  const authorization=request.headers.get('Authorization');
  const confirming=url.pathname.endsWith('/confirm');
  if(!confirming&&(!authorization?.startsWith('Bearer ')||authorization.length>16000))return response({error:'로그인이 필요합니다.'},401);
  ...
  try{
    const result=await fetch(env.SUPABASE_URL+'/functions/v1/anniversary-mail/email/'+url.pathname.split('/').pop(),{
      method:'POST',headers:{'Content-Type':'application/json',Origin:env.PUBLIC_SITE_URL,...(!confirming&&authorization?{Authorization:authorization}:{}),apikey:env.SUPABASE_PUBLISHABLE_KEY},body,signal:AbortSignal.timeout(55000),redirect:'manual'
    });
