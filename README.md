...
- 알림 이메일 변경·인증: 메일 버튼 한 번으로 로그인 없이 확인, 완료·만료·이미 처리된 링크 안내, 변경 요청 취소. 기존 주소는 인증 전까지 유지합니다. 뉴스 자동 발송은 추가 연결이 필요합니다. 메일 운영 상태는 [ANNIVERSARY-MAIL.md](ANNIVERSARY-MAIL.md)를 참고하세요.
