...
인증 메일 버튼을 누르면 별도 로그인이나 두 번째 버튼 없이 인증 결과가 표시됩니다. 256비트 무작위 토큰을 SHA-256 해시로 확인하며, 로그인 계정이나 요청 본문의 사용자 ID로 수신자를 정하지 않습니다. 주소 변경은 30분 안에 한 번만 수행합니다. 성공한 해시는 24시간 유효한 처리 영수증으로 남아 동일 링크를 다시 열 때 결과만 안내하고, 이전 주소로 되돌리지 않습니다. 계정 탈퇴 시 영수증도 삭제됩니다. 만료된 영수증은 다음 인증 요청에서 정리됩니다.

링크 토큰은 URL fragment로 전달하고 페이지가 읽은 뒤 주소에서 제거합니다. HTML GET만으로는 주소가 변경되지 않으며, 열린 페이지가 토큰을 POST해 확인합니다. 처리 중 새로고침을 복구할 수 있도록 같은 탭의 sessionStorage에 잠시 보관하고 결과가 확정되면 지웁니다. 새 인증 메일 발급 또는 로그인한 계정의 변경 요청 취소는 기존 링크를 무효화합니다. 새 주소 요청·취소에는 계속 로그인 인증이 필요합니다.

업데이트 배포: 기존 프로젝트에는 `202609230001_email_link_confirmation.sql`만 한 번 적용한 다음 메일 함수와 웹사이트를 배포합니다. 기본 스키마와 기념일 스케줄은 재실행하지 않습니다. 신규 설치는 기존 마이그레이션에 이어 이 파일까지 적용해야 합니다.
